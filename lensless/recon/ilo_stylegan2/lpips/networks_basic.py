...
class PNetLin(nn.Module):
    ...
    def forward(self, in0, in1, retPerLayer=False):
        ...
        for kk in range(self.L):
            feats0[kk], feats1[kk] = util.normalize_tensor(outs0[kk]), util.normalize_tensor(
                outs1[kk]
            )
...
class L2(FakeNet):
    def forward(self, in0, in1, retPerLayer=None):
        ...
        if self.colorspace == "RGB":
            (N, C, X, Y) = in0.size()
            value = torch.mean(
                torch.mean(torch.mean((in0 - in1) ** 2, dim=1).view(N, 1, X, Y), dim=2).view(
                    N, 1, 1, Y
                ),
                dim=3,
            ).view(N)
...
class DSSIM(FakeNet):
    def forward(self, in0, in1, retPerLayer=None):
        ...
        if self.colorspace == "RGB":
            value = util.dssim(
                1.0 * util.tensor2im(in0.data), 1.0 * util.tensor2im(in1.data), range=255.0
            ).astype("float")
        elif self.colorspace == "Lab":
            value = util.dssim(
                util.tensor2np(util.tensor2tensorlab(in0.data, to_norm=False)),
