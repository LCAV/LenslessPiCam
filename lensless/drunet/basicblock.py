...
class IMDBlock(nn.Module):
    ...
    def __init__(
        ...
        self.conv2 = conv(
            self.r_nc, in_channels, kernel_size, stride, padding, bias, mode, negative_slope
        )
        self.conv3 = conv(
            self.r_nc, in_channels, kernel_size, stride, padding, bias, mode, negative_slope
        )
        self.conv4 = conv(
            self.r_nc, self.d_nc, kernel_size, stride, padding, bias, mode[0], negative_slope
        )
        self.conv1x1 = conv(
            self.d_nc * 4,
            out_channels,
...
class ResidualDenseBlock_5C(nn.Module):
    def __init__(
        ...
        self.conv5 = conv(
            nc + 4 * gc, nc, kernel_size, stride, padding, bias, mode[:-1], negative_slope
        )
