...
def fc_simulation(img, mask, format="RGB", SNR=40):
    ...
    Y = (Y - Y.min()) / (Y.max() - Y.min())
    # Y = add_shot_noise(Y - Y.min(), snr_db=SNR) + Y.min()
    # Y = (Y - Y.min()) / (Y.max() - Y.min())
...
@hydra.main(
    version_base=None, config_path="../../configs", config_name="simulate_single_file_custom"
)
def simulate(config):
    ...
    mask_type = config.simulation.mask_type
