...
def ratio_to_key_length(ratio, bit_depth, n_pixel):
    return ratio * n_pixel / np.emath.logn(bit_depth, 2)


@hydra.main(
    version_base=None, config_path="../../configs/recon", config_name="recon_digicam_mirflickr_err"
)
def apply_pretrained(config):
    ...
    digicam_key_length = ratio_to_key_length(config.digicam_ratio, config.bit_depth, config.n_pixel)
    print(f"Digicam key length: {digicam_key_length} bits")
    ...
    if config.metrics_fp is not None:
        ...
        if config.only_save_idx:
            file_idx = config.save_idx
        else:
            file_idx = range(n_files)

        print(f"Processing {len(file_idx)} files...")
