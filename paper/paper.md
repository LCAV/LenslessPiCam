...
The DiffuserCam tutorial [@diffusercam] served as a great starting point to the present toolkit as it demonstrates that a working lensless camera can be built with cheap hardware: a Raspberry Pi, the Camera Module 2,[^1] and double-sided tape.
...
\autoref{fig:screen_examples} demonstrates reconstructed images of our students using `LenslessPiCam` and images that were projected on a monitor 40 cm away.
...
![Reconstructions of images displayed on a monitor. Each row represents a different algorithm. Image adapted from the report of a student group from our graduate-level signal processing course.[^3]](screen_examples.png){#fig:screen_examples width=100%}

[^3]: [infoscience.epfl.ch/record/291501](https://infoscience.epfl.ch/record/291501?ln=en)

As opposed to \autoref{fig:compare_cams} and \autoref{fig:screen_examples} which are back-illuminated objects (phone and screen respectively), \autoref{fig:in_the_wild} demonstrates reconstructions of objects illuminated with an external source.

![Reconstruction of objects 25cm away, illuminated with a lamp.](in_the_wild.png){#fig:in_the_wild width=90%}

\autoref{fig:tape_fov} demonstrates the field-of-view of the proposed system with double-sided tape. Objects are 40cm away.
...
- evaluate reconstructions on a variety of metrics: MSE, PSNR, SSIM, LPIPS [@zhang2018perceptual]. A tutorial can be found on [Medium](https://medium.com/@bezzam/image-similarity-metrics-applied-to-diffusercam-21998967af8d);
...
- our use of the real-valued FFT, which is possible since we are working with image intensities. Our convolver/deconvolver is implemented as an object - [`RealFFTConvolve2D`](https://github.com/LCAV/LenslessPiCam/blob/e31c9a7c8c87d30d4881b1123d849e7667b2e335/lensless/rfft_convolve.py#L16) -  that pre-computers the FFT of the PSF and supports SciPy and PyTorch backends.
...
Below is an example of how a reconstruction can be evaluated against an original image.[^4]

[^4]: Using [`scripts/compute_metrics_from_original.py`](https://github.com/LCAV/LenslessPiCam/blob/main/scripts/compute_metrics_from_original.py).
...
While `LenslessPiCam` could be used for collecting such a dataset with the proposed camera,[^5] the authors of [@monakhova2019learned] have already collected a dataset of 25'000 parallel measurements, namely 25'000 pairs of DiffuserCam and lensed camera images.[^6]
`LenslessPiCam` offers functionality to evaluate a reconstruction algorithm on this dataset, or a subset of it that we have prepared.[^7] 
...
[^5]: Using the remote display and capture scripts, i.e. [`scripts/remote_display.py`](https://github.com/LCAV/LenslessPiCam/blob/main/scripts/remote_display.py) and [`scripts/remote_capture.py`](https://github.com/LCAV/LenslessPiCam/blob/main/scripts/remote_capture.py) respectively.

[^6]: [waller-lab.github.io/LenslessLearning/dataset.html](https://waller-lab.github.io/LenslessLearning/dataset.html).

[^7]: Subset of [DiffuserCam Lensless Mirflickr Dataset](https://waller-lab.github.io/LenslessLearning/dataset.html)
...
to the subset we have prepared.[^8]

[^8]: Using `scripts/evaluate_mirflickr_admm.py`.
...
how the reconstruction changes as the number of iterations increase.[^9] The 
final reconstruction and outputed metrics are shown in 
\autoref{fig:dataset_single_file} and Table 4.

[^9]: Using `scripts/apply_admm_single_mirflickr.py`.
